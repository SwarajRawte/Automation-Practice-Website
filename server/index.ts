...
if (
  ...
  (productionSecret.length < 32 ||
    /change-me|docker-local-secret|local-development-secret/i.test(
      productionSecret,
    ))
)
  throw new Error(
...
const allowedOrigins = configuredAllowedOrigins(),
  corsOptions = createCorsOptions(allowedOrigins);
const app = express(),
  server = createServer(app),
  io = new Server(server, {
    cors: corsOptions,
    allowRequest(req, callback) {
      callback(null, requestOriginAllowed(req.headers.origin, allowedOrigins));
    },
  });
...
app.post("/api/test/users/:id/lock", testControlGuard, (q, r) => {
  ...
  if (!result.changes) return r.status(404).json({ error: "User not found" });
  disconnectUserSockets(Number(q.params.id));
  r.json({ id: Number(q.params.id), locked: Boolean(locked) });
});
app.post("/api/test/sessions/:userId/expire", testControlGuard, (q, r) => {
  const user = db
  ...
  disconnectUserSockets(Number(q.params.userId));
  r.json({
