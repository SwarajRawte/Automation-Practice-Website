...
function consumeToken(token: string, type: string) {
  const row = db
    .prepare(
      "SELECT t.*,u.email,u.name,u.role,u.verified,u.locked FROM auth_tokens t JOIN users u ON u.id=t.user_id WHERE t.token_hash=? AND t.type=? AND t.revoked=0",
    )
...
authRouter.post("/logout", (req, res) => {
  if (req.body.refreshToken)
    db.prepare("UPDATE auth_tokens SET revoked=1 WHERE token_hash=?").run(
