...
app.use((req: any, _res, next) => {
  req.user = {
    id: req.get("x-user") === "admin" ? 1 : 2,
    email: "test@testlab.local",
    name: "Test User",
...
test("shop catalog supports deterministic filtering, sorting, and product details", async () => {
  const catalog = await request(app)
    .get("/api/shop/products?category=Hardware&sort=price_desc")
    .expect(200);
...
test("wishlist and successful checkout persist a user order and reduce stock", async () => {
  ...
  const detail = await request(app)
    .get(`/api/shop/orders/${checkout.body.orderId}`)
    .expect(200);
...
test("order status events are scoped to the authenticated user's room", async () => {
  ...
  realtimeApp.use((req: any, _res, next) => {
    req.user = {
      id: 2,
  ...
  realtimeApp.use("/api", createPhase4Router(realtime));
  await request(realtimeApp)
    .post("/api/shop/checkout")
    .send({
  ...
  assert.equal(events[0].room, "user:2");
  assert.equal(events[0].event, "order-status");
});
...
test("mock payment decline, timeout, and invalid discount are deterministic", async () => {
  ...
  await request(app)
    .post("/api/shop/checkout")
    .send({ ...base, cardNumber: "4000000000000002" })
    .expect(402);
  await request(app)
    .post("/api/shop/checkout")
    .send({ ...base, cardNumber: "4000000000009995" })
    .expect(408);
  const discount = await request(app)
    .post("/api/shop/checkout")
    .send({ ...base, cardNumber: "4111111111111111", discountCode: "WRONG" })
    .expect(422);
  assert.equal(discount.body.code, "INVALID_DISCOUNT");
});

test("viewer role cannot mutate wishlist, checkout, or orders", async () => {
  await request(app)
    .post("/api/shop/wishlist/1")
    .set("x-role", "VIEWER")
    .expect(403);
  await request(app)
    .delete("/api/shop/wishlist/1")
    .set("x-role", "VIEWER")
    .expect(403);
  await request(app)
    .post("/api/shop/checkout")
    ...
    .expect(403);
  await request(app)
    .post("/api/shop/orders/1001/cancel")
    .set("x-role", "VIEWER")
    .expect(403);
});
...
test("network simulation and admin authorization are observable", async () => {
  const echo = await request(app)
    .post("/api/network/echo?case=phase4")
    .send({ hello: "world" })
    .expect(200);
  ...
  await request(app)
    ...
    .set("x-role", "ADMIN")
    .set("x-user", "admin")
    .expect("content-disposition", /orders-report.csv/)
    .expect(200);
  await request(app)
