...
Assert both UI feedback and backend state. Run tests independently, seed/reset during setup, avoid fixed sleeps, keep downloads in temporary directories, and capture trace/screenshot/video on failure. CI audits dependencies, typechecks, lints, runs the server integration suite, and verifies the production build.
